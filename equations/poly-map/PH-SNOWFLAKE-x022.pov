...
#macro weaveA( si, ii, seg )
	#local cc = c-1;
	// The current circle iterating ; 1+
	#local seq = ( max( ii, 1 ) / n );
	#local circle = floor( seq * c - 0.00000001 ) + 0;
	
	#local innerRadius = 2;
	#local depth = ( innerRadius/cc ) * circle;
	
	#local toggle = ceil( sin( ii*2 ) ) * 2 - 1;
	
	#local weave = innerRadius + ( cos( si * pi/1 ) ) * depth;
	weave
#end
...
#macro getVectorX( si, ii, seg )
	...
	#local X = X * weaveA( si, ii, seg );		//* pulseB( ii );
	X
#end
...
#macro getVectorY( si, ii, seg )
	...
	#local Y = Y * weaveA( si, ii, seg );		// * pulseB( ii );
	Y
#end
