...
#declare n = pow( harmonic, 5 ); 			// Number of Iterations

#declare getVectorX = function( si, ii, seg ){
	sin( si * pi * (2/seg) * 2  + pi ) * 3   + sin( ii/n * pi  * pow( 7, 4 ) + pi  ) * .1
}

#declare getVectorY = function( si, ii, seg ){
	cos( si * pi * (2/seg) * 2 + pi  ) * 3   + cos( ii/n * pi * pow( 7, 4 ) + pi  ) * .1
}
...
#macro equalateralArray( i )
	...
	#local sphereSize = sin( i/n * pi * segments * 2 + pi/7 ) * 0.02 + 0.02;
