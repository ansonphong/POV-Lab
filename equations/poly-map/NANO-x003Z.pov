...
#declare c = 2;		//6 * ml;		// Circles

#declare i = 0;						// Current Iteration
#declare n = h * 300; 				// Number of Iterations
...
///// OFFSET : DIFFUSE /////
#macro oDiffuse( ii )
	#local wave = cos( ii/n * pow( 6, 8 ) * pi*2 );
	#local st = 3;	// Steps
	abs( ceil( wave * st ) / st )
#end

///// CURRENT CIRCLE /////
#declare cc = function( ii ){
	ceil( ii/n * c )
}

///// CURRENT CIRCLE INTERVAL /////
#declare cci = function( ii ){
	cc( ii ) * ( floor( sin( ii/n * pi * 2 * 6 ) ) * 2 + 1 )
}

#declare oRot = function( ii ){
	pi/6 * (cc(ii) -1 )
}
...
#macro offsetX( si, ii, seg )
	#local o = cos( oSeq( ii ) * pi * 2 + oRot(ii)  ) * 5.2 * cci(ii) * oDiffuse(ii);
	o
#end

///// OFFSET : Y /////
#macro offsetY( si, ii, seg )
	#local o = sin( oSeq( ii ) * pi * 2 + oRot(ii)  ) * 5.2 * cci(ii) * oDiffuse(ii);
	o
#end

///// X /////
#macro getVectorX( si, ii, seg )
	#local X = ( sin( si * pi * ( 2 / seg ) * pow(6,2) + pi ) * coreScale );
...
#macro getVectorY( si, ii, seg )
	#local Y = ( cos( si * pi * ( 2 / seg ) * pow(6,2) + pi ) * coreScale );
