...
#declare n = pow( h, 5 ); 			// Number of Iterations

///// X /////
#declare getVectorX = function( si, ii, seg ){
	sin( si * pi * (2/seg) * 2  + pi ) * 5  - sin( ii/n * pi  * h * 2  +  (pi*h*2*((ii/n)/(h/2)))  ) * 1.5
}

///// Y /////
#declare getVectorY = function( si, ii, seg ){
	cos( si * pi * (2/seg) * 2 + pi  ) * 5  - cos( ii/n * pi * h * 2 +  (pi*h*2*((ii/n)/(h/2)))  ) * 1.5
}
...
#macro equalateralArray( i )
	...
	#local sphereSize = sin( i/n * pi * segments * 2 + pi/h - 1.1 ) * 0.06 + 0.06;
