...
///// HARMONIC /////
#declare h = 5;						// Haromonic
#declare c = 2;						// Circles
#declare i = 0;						// Current Iteration
#declare n = pow( h, 6 ); 			// Number of Iterations

///// X /////
#macro getVectorX( si, ii, seg )
	#local eqRot = (pi*h*c*((ii/n)/(h/c)));
	#local X = sin( si * pi * (2/seg) * c  + pi ) * 3   -  sin( ii/n * pi  * pow( h, 4 ) * 2  + eqRot  ) * .1;
	X
#end
...
#macro getVectorY( si, ii, seg )
	#local eqRot = (pi*h*c*((ii/n)/(h/c)));
	#local Y = cos( si * pi * (2/seg) * c + pi  ) * 3   -  cos( ii/n * pi * pow( h, 4 ) * 2  + eqRot  ) * .1;
	Y
#end
...
#macro equalateralArray( i )
	...
	#local sphereSize = sin( i/n * pi * segments * c + pi/(h/1.5)  ) * 0.01 + 0.015;
