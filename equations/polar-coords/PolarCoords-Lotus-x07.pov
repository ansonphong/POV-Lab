...
#declare c = pow( 2, 4 );				// Numer of times to circle
...
#while( i < n )
	...
	#declare shapes =  Lotus + sin( seq * pi * 3 + pi )/2 ;
	#declare Radius = 8 + shapes + weave * 4 + 4;
...
////////// POLAR COORDS : CONFIG //////////
#declare i = 0;							// Current Iteration
#declare c = pow( 2, 1 );				// Numer of times to circle
...
#while( i < n )
	...
	// This is a basic equasion for rendering lotus patterns
	#declare li = 	2;		// Initial Radius
	#declare lp = 	24; 		// Petals
	#declare lm = 	1;		// Multiplier
	#declare lo = 	4;		// In/Out Curve Ratio
	#declare lc = 	8 ;		// Curvature
	#declare lg = 	9; 		// Inward Gravity
	#declare lr = 	pi;		// Rotation
	#declare Lotus = li +lm*(((abs(cos(theta*(lp/2)+lr/2))*lo)-(abs(cos(theta*(lp/2)+pi/2+lr/2)))*lg)/(2+abs(cos(theta*(lp/2)*2+pi/2+lr))*lc));
	...
	#declare pulse = floor( seq * c - 0.0000001) / c;
	#declare frame = sin( pulse * pi );
	#declare shapes =  Lotus * 1 + sin(theta*lp/2+ pi/2 )*2; // + sin( seq * pi * 3 + pi )/2 ;
	#declare Radius = 24 + shapes + pulse*4;
...
#while( i < n )
	...
	#declare pulse = floor( seq * c - 0.0000001) / c;
	#declare frame = sin( pulse * pi  );
	#declare Radius = 32 + pow( abs( sin( theta * 12 ) ) , .33 ) * 6 * pulse; // + Polygon*16 ;
